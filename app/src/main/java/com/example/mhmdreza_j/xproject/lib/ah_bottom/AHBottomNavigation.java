...
import com.example.mhmdreza_j.xproject.lib.ah_bottom.notification.AHNotification;
import com.example.mhmdreza_j.xproject.lib.ah_bottom.notification.AHNotificationHelper;
...
public class AHBottomNavigation extends FrameLayout {
	...
	public void performOnClick(int position){
		if (views.get(position) != null) {
			views.get(position).performClick();
		}
	}
	/**
	...
	private void createSmallItems(LinearLayout linearLayout) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			TextView notification = (TextView) view.findViewById(R.id.bottom_navigation_notification);
			Drawable drawable = item.getDrawable(context);
	...
	private void updateSmallItems(final int itemIndex, boolean useCallback) {
		...
		if (currentItem == itemIndex) {
			if (tabSelectedListener != null && useCallback) {
				tabSelectedListener.onTabSelected(itemIndex, true);
				updateSelectedItems(currentItem, itemIndex);
				currentItem = itemIndex;
			}
			return;
		}
	...
	private void updateSelectedItems(int currentItem, int itemIndex) {
		if (currentItem == itemIndex) return;
		updateThisItem(currentItem, R.color.beige);
		updateThisItem(itemIndex, R.color.titleTextColor);

	}

	private void updateThisItem(int currentItem, int color) {
		if (currentItem < icons.size()){
			ImageView icon = icons.get(currentItem);
			icon.setColorFilter(getResources().getColor(color), PorterDuff.Mode.SRC_IN);
		}
	}
